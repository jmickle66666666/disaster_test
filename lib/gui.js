...
function transform3d(position)
{
    ...
    if (Input.mouseLeft && transformAxis != "n" && transformAxis != "1d") {
...
function transform1d(position, normal)
{
    var rotation = VMath.forwardToEuler(normal);
    var moved = false;
    var mouseRay = Physics.screenPointToRay(Input.mouseX, Input.mouseY);
    var hit = Physics.getCollisionRayModel(mouseRay, position, rotation, "tools/models/transformgizmoZ.glb");

    Draw.wireframe("tools/models/transformgizmoZ.glb", position, rotation, Color.gray, true, false, hit.hit);
    label(position.x + ", " + position.y + ", " + position.z);
    if (Input.mouseLeftDown && hit.hit) {
        var cross = vmath.normalise(vmath.cross(normal, Draw.getCameraTransform().forward));
        var planeNormal = vmath.cross(cross, normal);
        transformPlane = {position:position, normal:planeNormal};
        transform3dStart = vmath.sub(Physics.getCollisionRayPlane(mouseRay, transformPlane).position, position);
        transformAxis = "1d";
        log(transform3dStart.x + ", " + transform3dStart.y + ", " + transform3dStart.z);
        hovered = true;
    }

    if (Input.mouseLeft && transformAxis == "1d") {
        var currentPosition = vmath.sub(Physics.getCollisionRayPlane(mouseRay, transformPlane).position, transform3dStart);
        currentPosition = VMath.closestPointOnLine(position, normal, currentPosition);
