...
function bbox(x, y, width, height, object)
{
    colliders.push(
        {
            x: x, y: y, width: width, height: height, object: object
        }
    );
}
    ...
    for (var i = 0; i < colliders.length; i++)
    {
        ...
        for (var j = i + 1; j < colliders.length; j++)
        {
            ...
            if (c1.x + c1.width >= c2.x && c1.y + c1.height >= c2.y && c1.x < c2.x + c2.width && c1.y < c2.y + c2.height)
            {
                ...
                if (c1.object != null && c1.object.collide != undefined) { 
                    c1.object.collide(c2.object);
                }
                if (c2.object != null && c2.object.collide != undefined) {
                    c2.object.collide(c1.object);
                }
            }
        }
    }
